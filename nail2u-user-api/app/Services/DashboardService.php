...
class DashboardService extends BaseService
{
    ...
    {
        try {
            $response = Helper::httpRequest('get', config('constants.ADMIN_APP_URL') . config('constants.GET_CAROUSEL_IMAGES_API'), null, []);
            if ($response->_metadata->httpResponseCode == GlobalApiResponseCodeBook::RECORD_NOT_EXISTS['httpResponseCode'])
                return GlobalApiResponseCodeBook::RECORD_NOT_EXISTS['outcomeCode'];
            if ($response->_metadata->httpResponseCode == GlobalApiResponseCodeBook::SUCCESS['httpResponseCode'])
                return $response->records;
            return false;
        } catch (Exception $e) {
            $error = "Error: Message: " . $e->getMessage() . " File: " . $e->getFile() . " Line #: " . $e->getLine();
            Helper::errorLogs("DashboardService: getCarouselImages", $error);
            return false;
        }
    }
    ...
    {
        try {
            $booking = Booking::select('id', 'artist_id', 'client_id', 'started_at',)
                ->with([
                    'BookingService:id,name',
                    'User:id,username,image_url,cv_url',
