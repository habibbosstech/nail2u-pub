...
class AuthService extends BaseService
{
    public function register($request)
    {
        try {
            ...
            $otp_value = rand(1223, 5556666);

            $otp = new OTP();
            $otp->user_id = $user->id;
            $otp->otp_value = $otp_value;
            $otp->save();
            ...
            $this->dispatchOTP($request->phone_no, $otp_value);
    ...
    public function login($request)
    {
        try {
            ...
            if ($user->phone_verified_at == null || $user->email_verified_at == null )
                return Helper::returnRecord(GlobalApiResponseCodeBook::PHONE_NOT_VERIFIED['outcomeCode'], []);
...
//            return $data;
    ...
    public function dispatchOTP($phone_no, $otp)
    {
        try {
            $phone_no = '+923494056872';
            $sid = env('TWILIO_SID');
            ...
            $message = $twilio->messages
                ->create($phone_no,
                    array(
                        "messagingServiceSid" => "MG8d8ebc604af7ab2c8126554ee7e5f5c6",
                        "body" => $otp
                    )
                );
            ...
            DB::rollBack();
            $error = "Error: Message: " . $e->getMessage() . " File: " . $e->getFile() . " Line #: " . $e->getLine();
            Helper::errorLogs("User: AthService: dispatchOTP", $error);
            return false;
        }
    }
    ...
    {
        try {
            $otp = OTP::where('otp_value', '=', $otp)->get(['user_id'])->toArray();

            if (isset($otp[0]['user_id'])) {
                User::where('id', $otp[0]['user_id'])->update(['user_verified_at' => Carbon::now()]);
            ...
            DB::rollBack();
            $error = "Error: Message: " . $e->getMessage() . " File: " . $e->getFile() . " Line #: " . $e->getLine();
            Helper::errorLogs("User: AthService: verifyOtp", $error);
            return false;
        }
    }
}
