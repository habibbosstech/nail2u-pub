...
class DashboardController extends Controller
{
    ...
    {
        $carousel_images = $this->dashboard_service->getCarouselImages();

        if ($carousel_images === GlobalApiResponseCodeBook::RECORD_NOT_EXISTS['outcomeCode'])
            return ($this->global_api_response->error(GlobalApiResponseCodeBook::RECORD_NOT_EXISTS, "No carousel images found!", []));

        if (!$carousel_images)
            return ($this->global_api_response->error(GlobalApiResponseCodeBook::INTERNAL_SERVER_ERROR, "Carousel images did not fetched!", $carousel_images));

        return ($this->global_api_response->success(1, "Carousel images fetched successfully!", $carousel_images));
    }
