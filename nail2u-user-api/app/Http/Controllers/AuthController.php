...
class AuthController extends Controller
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        if ($login['outcomeCode'] === GlobalApiResponseCodeBook::PHONE_NOT_VERIFIED['outcomeCode'])
            return ($this->global_api_response->error(GlobalApiResponseCodeBook::PHONE_NOT_VERIFIED, "Your phone number is not verified!", $login['record']));
    ...
    {
        $verify_otp = $this->auth_service->verifyOtp($otp);

        if ($verify_otp['outcomeCode'] === GlobalApiResponseCodeBook::RECORD_NOT_EXISTS['outcomeCode'])
            return ($this->global_api_response->error(GlobalApiResponseCodeBook::RECORD_NOT_EXISTS, "OTP not found/expired!", $verify_otp['record']));

        return $this->global_api_response->success(1, "User verified successfully!", []);
